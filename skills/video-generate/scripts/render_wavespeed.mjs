...
async function submitWavespeed(
  token,
  { prompt, duration, resolution, enableWebSearch, generateAudio, image, useTextToVideo },
) {
  const payload = {
    duration,
...
async function main() {
  ...
  if (!parsed.apiKey) {
    console.error(
      "Error: missing WAVESPEED_API_KEY. Provide via --api-key or set environment variable.",
    );
    process.exit(1);
  }
  ...
  for (let index = 0; index < prompts.length; index++) {
    ...
    const prediction = await submitWavespeed(parsed.apiKey, {
      ...
      enableWebSearch:
        item.enable_web_search !== undefined ? item.enable_web_search : parsed.enableWebSearch,
    ...
    console.log(
      `[${index + 1}/${prompts.length}] Shot ${shotId} prediction ID: ${requestId}. Polling status...`,
    );
    const status = await pollPrediction(parsed.apiKey, requestId);
    if (status.status !== "completed") {
      console.error(
        `Error: Wavespeed rendering failed or was canceled for shot ${shotId}:`,
        status,
      );
      process.exit(1);
    }
    ...
    console.log(
      `[${index + 1}/${prompts.length}] Shot ${shotId} rendering complete. Retrieving result URL...`,
    );
