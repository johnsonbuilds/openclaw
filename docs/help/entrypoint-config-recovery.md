...
截至目前，以下场景已经在实际 SaaS / 容器部署流程中完成验证：

- 旧实例升级到最新镜像
  - 已有 `openclaw.json` 时走 `update-llm-only`
  - 启动成功
  - 启动后会建立或刷新 `openclaw.json.bak`
- 全新部署
  - 空数据目录时走 `overwrite-defaults`
  - 成功生成新的 `openclaw.json`
  - gateway 正常启动
- 当前配置无效、`.bak` 有效
  - 入口脚本能检测到无效配置
  - 自动从 `.bak` 恢复
  - 会生成 `openclaw.json.invalid.<UTC 时间戳>` 快照
  - 恢复后服务可继续正常启动
- 外部直接把 `openclaw.json` 改成新的、仍然合法的配置
  - gateway 可以正常处理该配置变更
  - `.bak` 会自动刷新为新的有效配置，作为新的恢复基线
- 更新现有配置的 LLM 字段后重启
  - 入口脚本会保留现有配置主体，仅同步 LLM 字段
  - 更新后的模型配置可生效
  - `.bak` 会随之更新
